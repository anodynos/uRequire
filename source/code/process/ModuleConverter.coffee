...
@param {Dependency@reporter} @reporter an optional `Dependency@reporter`

@return {String} the converted module js
###
class ModuleConverter
  
  constructor: (@modyle, @oldJs, @bundleFiles, @options, @reporter)->
#    @initialize()
  ...
  convert:->
    ...
    if _.isEmpty @moduleInfo
      ...
      templateInfo = #
        version: if (typeof version is 'undefined') then @options.version || 'unknown' else version # 'var version = xxx' added by grunt concat @ .js top. (alt use @options.version)
...
module.exports = ModuleConverter
