...
module.exports =
  ...
  class UModule
    ...
    _constructor: (
      @filename    # the full filenname of this module, eg 'models/PersonModel.coffee'
      @_sourceCode      # String of source code eg coffee (or .js)

      @bundleFiles # todo: 'bundle' where it belongs

      @options     # the options used for the compilation #@todo : make @options a global ? :-)
      @reporter    # @todo refactor/globalize!
    )->
      @adjustModuleInfo()
    ...
    ### Module source code, as is (might be coffee, coco, livescript, typescript etc) ###
    @property sourceCode:
      enumerable: false
      get: -> @_sourceCode
      set: (src)->
    ...
    convert: (template = @options.template) ->
      if @isConvertible
    ...
    reportDeps: ->
      if @reporter
        for repData in [ (_.pick @depenenciesTypes, @reporter.interestingDepTypes) ]
          @reporter.addReportData repData, @filename
        ...
        if @isConvertible
          if _.isEmpty @_globalDepsVars # reset at @adjustModuleInfo()
            for d, idx in @arrayDependencies
              d = new Dependency d, @filename, @bundleFiles
    ...
    adjustModuleInfo: ->
      ...
      if _.isEmpty mi
          ...
          asyncDeps ] = for strDepsArray in [ # @todo: do we need to replaceAsynchRequires ?
             ...
             mi.asyncDependencies
              ...
              for strDep in (strDepsArray || [])
                dep = new Dependency strDep, @filename, @bundleFiles
                deps.push dep
        ...
        if not (_.isEmpty(mi.arrayDependencies) and @options.scanAllow and not mi.rootExports)
        ...
        mi.nodeDependencies = if @options.allNodeRequires then mi.arrayDependencies else (d.name() for d in arrayDeps)
