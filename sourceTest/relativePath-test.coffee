console.log 'relativePath-test started'

chai = require 'chai'
relativePath = require "../build/relativePath"

assert = chai.assert
expect = chai.expect

describe "relativePath(from, to)", ->

  it "should work in simple cases", ->
    from  = 'common\\a\\b'
    to    = 'common\\d\\e'

    expect(relativePath from, to).to.equal "../../d/e"

    [from, to] = [to, from]
    expect(relativePath from, to).to.equal "../../a/b"

  it "should blank on identical path", ->
    from = to = 'common/a/b'
    expect(relativePath from, to).to.equal ""

  it 'should calc go-back & forth paths', ->
    from  = "/y/work/p/project/sourceTest/code/main"
    to    = "/y/work/p/project/source/code/main//"

    expect(relativePath from, to).to.equal "../../../source/code/main"

    [from, to] = [to, from]
    expect(relativePath from, to ).to.equal "../../../sourceTest/code/main"
    @
  ...
  it "should handle 'no path found' will null", ->
    expect(relativePath 'junk\\a\\b', 'bin\\a\\b').to.be.a 'null'
  ...
  it 'should handle go-back/go forwards only correctly', ->
    ...
    [from, to] = [to, from]
    expect(relativePath from, to).to.equal "a/b/c/d"

  it 'should go from `root` to `to` and vise versa, with dot4Current', ->
    from =  "$"
    to =    "$/work/p"

    expect(relativePath from, to, dot4Current:true).to.equal "./work/p"

    [from, to] = [to, from]
    expect(relativePath from, to, dot4Current:true).to.equal "../.."

  it 'should consume pointless back and forths', ->
    from =  "/y/work/./p/./a/../b/./c/../../../" # I am back in work
    to =    "/y/work/p/./d/."

    expect(relativePath from, to).to.equal "p/d"
  ...
  it 'should handle it all together', ->
    ...
    expect(relativePath from, to).to.equal "p/m/e/f"
  ...
  it 'should go off cliff (BEFORE the common path), ONLY if `to` says so', ->
    ...
    expect(relativePath from, to).to.equal "../../../../../f/g"

    [from, to] = [to, from]
    expect(relativePath from, to).to.equal null

  it 'should navigate back and forth correctly', ->
    from =  "/work1/../work2/g"
    to =    "/work1/p/a" #

    expect(relativePath from, to).to.equal '../../work1/p/a'
